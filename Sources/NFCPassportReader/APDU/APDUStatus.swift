...
struct APDUStatus: Equatable, Sendable {
    ...
    var readerError: NFCPassportReaderError? {
        ...
        return .ResponseError(description, sw1, sw2)
    }
    ...
    var description: String {
        ...
        return Self.descriptions[sw1]?[sw2] ?? "Unknown error - sw1: 0x\(binToHexRep(sw1)), sw2 - 0x\(binToHexRep(sw2)) "
    }

    private static let descriptions: [UInt8: [UInt8: String]] = [
        0x62: [
...
extension ResponseAPDU {
    ...
    func ensureSuccess() throws {
        if let readerError = status.readerError {
            throw readerError
        }
    }
